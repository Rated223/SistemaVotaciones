            ...
            <div class="col-11 my-4">
                ...
                <?php
                    ...
                    if(isset($_GET['buscar'])){
                        $re2 = $mysqli->query("SELECT *, DATE_FORMAT(fecha_inicio_votacion, '%d-%m-%Y') AS fecha_inicio_votacion, DATE_FORMAT(fecha_final_votacion, '%d-%m-%Y') AS fecha_final_votacion FROM `votacion` INNER JOIN `votos_emitidos` ON votacion.id_votacion = votos_emitidos.id_votacion INNER JOIN `usuarios` ON votos_emitidos.id_usuario = usuarios.id_usuario WHERE usuarios.nombre_usuario = '".$_SESSION['usuario']."' && nombre_votacion  LIKE '%".$_GET['buscar']."%' ORDER BY `votacion`.`id_votacion` DESC LIMIT $empieza, $cantidad_resultados");
                    } else {
                        $re2 = $mysqli->query("SELECT *, DATE_FORMAT(fecha_inicio_votacion, '%d-%m-%Y') AS fecha_inicio_votacion, DATE_FORMAT(fecha_final_votacion, '%d-%m-%Y') AS fecha_final_votacion FROM `votacion` INNER JOIN `votos_emitidos` ON votacion.id_votacion = votos_emitidos.id_votacion INNER JOIN `usuarios` ON votos_emitidos.id_usuario = usuarios.id_usuario WHERE usuarios.nombre_usuario = '".$_SESSION['usuario']."' ORDER BY `votacion`.`id_votacion` DESC LIMIT $empieza, $cantidad_resultados");
                    }
